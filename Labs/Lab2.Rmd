...
Use `group_by(experience)` along with the pipe `%>%` to output the most common amount of years of experience along with the number of occurrences found in the data. The most common value for years of experience is __9___ and occurs __654___ times. Fill in the blanks with the correct answers, and change `eval=True` to `eval=False` and print out the output that led you to your answer.
...
Next, modify the code in Question 1.2 to display the maximum level of schooling and the number of workers in the data that had that number of schooling. The maximum number of years in school was _12___ years which occurred __1188___ times in our sample. Fill in the blanks with the correct answers.
...
The years of experience seem to __decrease___ (increase/decrease) as the years of schooling increases. Is this what you expected to see? __no__ (yes/no). 
...
* Maybe, people who go to school longer have less work experience, especially at the same time.
...
* The first calculation is incorrect and cannot be used to determine statistical significance. The second caclulation does show statistical significance because the 95% confidence intervals do not overlap.
...
Use `geom_point()` along with the option `color=sex` to overlay scatter plots. Does there seem to be a clear distinction between female and male regarding this relationship? ___no___ (yes/no).
...
Repeat the graphic created in Question 4 replacing `x=experience` with `x=school`. Does there seem to be a clear distinction between female and male regarding this relationship? ____yes__ (yes/no).
...
* For both males and females, higher levels of education associated with more schooling is associated with higher wages. Wage increases with experience plateau after a certain point for those who have less schooling. This might suggest that individuals with high schooling and high experience may earn the highest wages. Men seem to have slightly higher wages across most experience and schooling combined, and women in general have similar patterns, but with lower wages compared to their schooling and experience.
...
* The female facet seems to be darker, especially when it comes to combinations of experience and schooling. This might imply that the variation in wages for women is smaller for men with the same combinations between schooling and experience. The male facet seems to be lighter, which implies more variability in wages.
...
* Workers with 5 years of experience and 11 years of schooling; if a male tile is lighter than a female tile, it might suggest wages varying more widely among men than women with the same creds. It may imply that men are likely to recieve higher wages, or in contrast, lower wages, but womens wages are more in that middle area (the tighter clustering). These could be due to wage-gap concerns.
...
# still confused on converting to numeric from str
#?gather
#?rename
```
...
* Lebron James would need to distribute the money based on resource allocation on a need-based basis. In the boxplot, we can see that most schools improved scores in 2018, but some were still lagging in performance. The schools that are having poorer performance should get more funding because lower funding may be tied to not having enough resources or teachers. This may be why those schools are under performing, while the schools with better performance should receive the same, since I would assume they had the resources they already needed. At the same time, I think we should still reward improvement, so maybe a one-time bonus should be given to the schools that are performing well.
...
* The first plot shows raw scores and shows that students increased math scores in those years, while the second plot shows that based on percentiles, the students did not make a significant improvement based on ranking (versus peers/other districts). So, test scores improved, but test scores did not improve compared to eachother. This could possibly mean that the district chose to prepare for examinations to improve test scores, but did not give special attention to already-low performing test takers. In other words, students were able to learn how to take a test by practice, but not actually learn the material or apply it. This is especially emphasized by the similar distribution seen in the second plot, and growth in the first plot cannot be generalized as growth of the whole.
