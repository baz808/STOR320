...
```{r}
ggplot(data=msleep2) + 
  geom_boxplot(aes(x=vore,y=awake),fill=c("red","blue","green","purple")) + #vector coloring each boxplot
...
ggplot(data=msleep2) + 
  ...
  scale_x_discrete(limits=c("carni","herbi"),labels=c("Carnivore","Herbivore")) + # only wanted to look at carnivore and herbivore, limits are listing only the values we want to see and labels relabeling those values 
  guides(color=guide_legend(title="Conservation \nStatus")) + # \n recongifures the Status under the Conservation for aesthetic purposes 
  theme_classic()
...
ggplot(data=msleep2,aes(x=vore,y=conservation)) + 
  geom_tile(aes(fill=sleep_total)) + # introducing third variable to represent tiles/contour
  scale_fill_gradient(low="deepskyblue1",high="white")+
  theme_classic() + 
  scale_x_discrete(label=c("Carnivore","Herbivore","Insectivore","Omnivore")) +
  theme(axis.text.x=element_text(angle=45,vjust=0.5))+ # angles and adjusts variable names on the x axis
  xlab("")+ylab("") + 
  ggtitle("Total Sleep for Combinations of Conservation Status and Diet")

# in general not a sufficient plot: this visual does not clearly represent missing values, the sleep_total is not represented clearly by the numbers (we did not summarize the data we do not know what the numbers mean (avg, median, sd, etc.))
```
